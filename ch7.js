// JavaScript Document pg 471
"use strict";

// track date the user has selected or computer is using
...
function displayCalendar(whichMonth) {
	...
	//navigate to the previous or next month using buttons on calander widget
	if (whichMonth === -1) {
	...
	//set the number of days in selected month
	...
	//clear existing HTML table contents
	dateCells = document.getElementsByTagName("td");
	for (var i = 0; i < dateCells.length; i++) {
		 //clearing existing table dates
		 dateCells[i].innerHTML = "";
		 dateCells[i].className = "";
	}
	//add new dates to appropriate alandar table cells
	for (var i = dayOfWeek; i < daysInMonth + dayOfWeek; i++) {
		//add dates to days cells
		dateCells[i].innerHTML = dateObject.getDate();
		dateCells[i].className = "date";
		if (dateToday < dateObject) {
			dateCells[i].className = "futuredate"; //modified in CSS styles
		}
...
function selectDate (event) {
	...
	hideCalendar();
	updateTotalCost(); //when the user selects a date, this statement calls the updateTotalCost function
	countdown = setInterval(updateCountdown, 1000);//repeatedly calls the function every 1000 milliseconds pg489
	document.getElementById("countdownSection").style.display = "block";//sets value of global cntdn var, to be canceled later
}
function hideCalendar() {
	document.getElementById("cal").style.display = "none";
}
// two functions that display previous and next buttons for the months
function prevMo() {
	displayCalendar(-1);
}
function nextMo() {
...
function updateCountdown() {
	var dateToday = new Date(); //sets current date and time
	...
	var dateTo = Date.UTC(dateObject.getFullYear(),
						 dateObject.getMonth(), 
						 dateObject.getDate(), 19, 0, 0); //8:00pm UTC
	if ((dateTo - dateFrom) < 1000) { //time will be less than 0 when setinterval runs next pg 489
	...
	document.getElementById("countdown").innerHTML += ":" + hoursUntil; //pg 486
	//minutes
	...
	document.getElementById("countdown").innerHTML += ":" + secondsUntil; //pg487
}
...
function createEventListeners() {
	...
	//calling the hide calendar function
	...
	//calls the preMo and nextmo functions
